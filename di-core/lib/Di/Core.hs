...
log
   ...
   -- ^ Log message.
   -> m ()
log di l = log' (\stm -> liftIO (STM.atomically stm)) di l
{-# INLINE log #-}
...
flush = \di -> flush' (\stm -> liftIO (STM.atomically stm)) di
{-# INLINE flush #-}
...
throw'
   ...
   -> m a
throw' nat di = \e ->
   case di_logex di mempty (Ex.toException e) of
      Nothing -> nat $ STM.throwSTM e
      Just logex -> do
         -- If logging throws an exception, ignore it so that `e` is propagated. This
         -- means that if there's something wrong with `logex`, we may not notice.
         nat $ STM.catchSTM logex $ \(_ :: Ex.SomeException) -> pure ()
         -- If logging throws an exception, ignore it so that `e` is propagated. This
...
-- @
--
-- If according to the 'onException' policy the exception is not logged,
-- then this function behaves as 'Ex.throwIO'.
--
-- @
-- 'throw' ('onException' ('const' 'Nothing') di)  ==  'liftIO' . 'Ex.throwIO'
-- @
throw
   :: (Ex.Exception e, MonadIO m)
   => Di level path msg
   -> e
   -- ^ 'Ex.Exception'.
   -> m a
throw di = throw' (\stm -> liftIO (STM.atomically stm)) di
{-# INLINE throw #-}
...
-- @
--
-- Notice that the @level@ and @msg@ resulting from @g@ are discarded, yet its
-- policy regarding whether to log or not is preserved in the same way
...
onException f = \di0 ->
   di0
      { di_logex = \ps0 se -> do
         ...
         (l, m) <- f se
         let di1 = foldl' (flip push) di0 ps0
         Just (log' id di1 l m)
      }
